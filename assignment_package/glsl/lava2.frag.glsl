...
{
 return texture(u_RenderedTexture,coords + vec2(dx, dy));
}

// returns pixel color
float IsEdge(in vec2 coords)
{
  float dxtex = 1.0 /float(textureSize(u_RenderedTexture,0)) ;
  float dytex = 1.0 /float(textureSize(u_RenderedTexture,0));
  float pix[9];
  int k = -1;
  float delta;
...
void main()
{
  ...
  {
    vec3 colorOrg = texture(u_RenderedTexture, uv).rgb;
